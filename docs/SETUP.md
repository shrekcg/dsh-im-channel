...
运行 `npm run features` 查看**每项功能的配置状态**，一眼看清哪些配了、哪些没配、没配怎么配：

```bash
npm run features         # 完整清单 (含每项的状态/配置方法/验证方式)
npm run features:json    # JSON 输出 (供程序读取)
```

清单覆盖：常驻服务、飞书工具(40个)、多媒体、流式输出、图片视觉理解、表情反馈、交互卡片回调、多账号。

**在飞书对话中查询**：直接问"功能清单"或"哪些功能配了"，agent 会自动运行并解读结果。

### 未配置项的处理

| 功能 | 未配置时 | 如何配置 |
|---|---|---|
| 表情反馈 | 给 bot 回复加表情无感知 | 后台事件订阅添加 `im.message.reaction.created_v1` |
| 交互卡片回调 | 卡片按钮无响应 | 后台事件与回调→回调配置订阅 `card.action.trigger` |
